...
class ClienteMODBUS():
    ...
    def __init__(self, server_addr, porta, device_id, broker_addr, broker_port, scan_time=0.2):
        ...
        self._cliente = ModbusClient(
            host=server_addr, port=porta, unit_id=device_id)
                    ...
                    if tipo == 3 or tipo == 4:
                        ...
                        if val == 1:  # valores decimais
                            ...
                            try:
                                ...
                                self._threadread = Thread(
                                    target=self.readThread, args=(tipo, 1, addr, leng, func,))
                                ...
                                try:
                                    ...
                                    for i in range(0, int(nvezes)):
                                        ...
                                        modbusValues = self.lerDado(
                                            int(tipo), int(addr), leng)
                                        print(modbusValues)
                                        self.mqttPublish(
                                            topic="test/status", msg=f"Leitura {i+1} {func}: {modbusValues}")
                            ...
                            try:
                                self._readingthread = True
                                self._threadread = Thread(
                                    target=self.readThread, args=(tipo, 2, addr, leng, func,))
                                self._threadread.start()
                            except Exception as e:
                                print('ERRO: ', e.args)
                                try:
                                    ...
                                    for i in range(0, int(nvezes)):
                                        ...
                                        modbusValues = self.lerDadoFloat(
                                            int(tipo), int(addr), leng)
                                        print(modbusValues)
                                        self.mqttPublish(
                                            topic="test/status", msg=f"Leitura {i+1} {func}: {modbusValues}")
                            ...
                            try:
                                for i in range(0, int(nvezes)):
                                    print(f'Leitura {i + 1}:', end='')
                                    modbusValues = self.lerDadoFloatSwapped(
                                        int(tipo), int(addr), leng)
                                    print(modbusValues)
                                    self.mqttPublish(
                                        topic="test/status", msg=f"Leitura {i+1} {func}: {modbusValues}")
                    ...
                    if int(tpconfig) == 1:
                        ...
                        while True:
                            ...
                            if config not in '12345':
                                print(
                                    'Enter a valid configuration type.. (1, 2, 3, 4 or 5)')
                                sleep(0.5)
                            else:
                                break
                        if int(config) == 1:
                            ...
                            while True:
                                ...
                                if 0 <= int(iddevice) < 256:
                                    ...
                                    print(
                                        'Device ID must be an integer between 0 and 256.', end='')
                                    sleep(0.5)
                            try:
                                self._cliente.close()
                        ...
                        while True:
                            ...
                            if config not in '123':
                                print(
                                    'Enter a valid configuration type.. (1, 2 or 3)')
                                sleep(0.5)
                            else:
                                break
                        if int(config) == 1:
                            ...
                            try:
                                ...
                                try:
                                    if self._client_mqtt.connect(self._broker_addrs, self._broker_port, 60) != 0:
                                        ...
                                        print(' --> OK')
                                        print(
                                            f'Broker IP successfully changed to {ipserv}!!\n')
                                        self._status_conn_mqtt = True
                                        sleep(0.2)
                                except Exception as e:
                                    print('ERROR: ', e.args)
                                    ...
                                    print(
                                        'Following without connection with MQTT Broker..')
                                    self._status_conn_mqtt = False
                                self._client_mqtt.disconnect()
                            except Exception as e:
                                print('ERROR: ', e.args)
    ...
    def mqttPublish(self, topic, msg):
        ...
        try:
            ...
            print(
                'Error when trying to publish to broker, please check IP address and port...')
            self._status_conn_mqtt = False
        ...
        if display == 1:
            try:
                ...
                while self._readingthread:
                    ...
                    if self._status_conn_mqtt:
                        self.mqttPublish(
                            topic="test/status", msg=f"Read {i+1} - ({addr}:{leng}) {func}: {modbusValues}")
                    sleep(self._scan_time)
                    i += 1
                    sleep(0.2)
            except Exception as e:
            ...
            try:
                ...
                while self._readingthread:
                    ...
                    if self._status_conn_mqtt:
                        self.mqttPublish(
                            topic="test/status", msg=f"Read {i+1} - F03.HR (350-369): {modbusValues}")
                    sleep(self._scan_time)
                    i += 1
                    sleep(0.2)
            except Exception as e:
            ...
            try:
                ...
                while self._readingthread:
                    ...
                    if self._status_conn_mqtt:
                        self.mqttPublish(
                            topic="test/status", msg=f"Read {i+1} - {func}: {modbusValues}")
                    sleep(self._scan_time)
                    i += 1
                    sleep(0.2)
            except Exception as e:
